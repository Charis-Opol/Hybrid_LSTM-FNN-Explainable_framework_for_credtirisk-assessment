#!/usr/bin/env python
"""Evaluation comparison: hybrid, logistic regression, XGBoost, vanilla LSTM,
and both transformer variants.

Trains and evaluates six models with stratified 5-fold CV using pooled
...
each model family's default settings:
  ...
  4. Vanilla LSTM (untuned)            -- plain recurrent baseline, temporal-only
  5. Transformer-Encoder Hybrid        -- GRU replaced by self-attention
  6. Cross-Attention Fusion Hybrid     -- static queries temporal sequence
...
Outputs, all under one folder:
  ...
  - pr_curves_overlay.png  all six PR curves + random baseline on one plot
...
def write_experiment_summary(
    ...
    lines = [
        "Evaluation Comparison: Hybrid vs. Logistic Regression vs. XGBoost vs. "
        "Vanilla LSTM vs. Transformers",
        "=" * 80,
...
def main() -> None:
    ...
    # --- 5-fold CV: all six models, none hyperparameter-tuned ---
    ...
    LOGGER.info("Running 5-fold CV: vanilla LSTM (untuned)")
