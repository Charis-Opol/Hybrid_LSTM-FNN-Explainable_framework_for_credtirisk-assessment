...
def build_transformer_hybrid_model(
    ...
    # Computed directly from the raw input rather than via a Masking layer,
    # since automatic mask propagation breaks once the (batch-less)
    # positional embedding tensor enters the graph below.
    ...
    # FIX: Used standard element-wise addition operator instead of tf.keras.layers.Add().
    # This natively broadcasts across the batch dimension without tricking Keras 
    # into overriding the dynamic batch size with the temporal sequence length.
    x = x + position_embeddings

    # Broadcastable to (batch, query_len, key_len): masks out padded key
    ...
    pooled = tf.keras.layers.Lambda(
        lambda inputs: (
            tf.reduce_sum(inputs[0] * tf.cast(inputs[1], tf.float32)[..., tf.newaxis], axis=1)
            / tf.maximum(
                tf.reduce_sum(tf.cast(inputs[1], tf.float32)[..., tf.newaxis], axis=1), 1e-6
            )
        ),
