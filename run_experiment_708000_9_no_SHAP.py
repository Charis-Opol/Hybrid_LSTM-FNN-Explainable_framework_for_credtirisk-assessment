...
Tunes hyperparameters for the XGBoost, logistic regression, and hybrid
models, then evaluates all four models (tuned XGBoost, tuned logistic
regression, tuned hybrid, and the untuned vanilla LSTM baseline) with
stratified 5-fold CV using pooled out-of-fold predictions -- required
given the very small positive class (~2%, ~70 positive borrowers
total), where a single train/val/test split does not give stable
metrics. A single-split train/evaluate run of the (untuned) hybrid
model is kept at the end for continuity with earlier experiments.

Pipeline:
  1. Load data, engineer features
  2. Hyperparameter tuning: XGBoost, logistic regression, hybrid model
  3. 5-fold CV, pooled OOF metrics: tuned XGBoost, tuned logreg,
     tuned hybrid, untuned vanilla LSTM
  4. Model comparison chart + markdown table
  5. Single-split hybrid train/evaluate (continuity with experiments 1-7)
"""
...
OUTPUT_DIR = (
    PROJECT_DIR
    / "models"
    / "uganda_mobile_money_708000_9_no_SHAP"
)
...
def main() -> None:
    ...
    # --- Single-split training/evaluation for the untuned hybrid model (continuity) ---
