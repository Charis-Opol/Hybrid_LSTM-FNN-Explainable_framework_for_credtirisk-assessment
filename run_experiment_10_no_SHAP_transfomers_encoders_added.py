...
Tunes hyperparameters for XGBoost, logistic regression, and the hybrid
model, then evaluates six models with stratified 5-fold CV using pooled
out-of-fold predictions (required given the very small positive class,
~2.3%, ~70 positive borrowers total):

  1. Hybrid LSTM-GRU-Attention-FNN (tuned)      -- original architecture
  2. XGBoost (tuned)                             -- tree baseline
  3. Logistic Regression (tuned)                 -- linear baseline
  4. Vanilla LSTM (untuned)                       -- plain recurrent baseline
  5. Cross-Attention Fusion Hybrid (untuned)      -- static queries temporal sequence
  6. Transformer-Encoder Hybrid (untuned)         -- GRU replaced by self-attention

A single-split train/evaluate run of the original (untuned) hybrid
model is kept at the end for continuity with earlier experiments.
"""
...
from kfold_cv_transformer_encoder import run_kfold_cv_transformer_encoder
...
from tune_hybrid_model import tune_hybrid_model
from tune_logistic_regression import tune_logistic_regression
...
OUTPUT_DIR = (
    PROJECT_DIR
    / "models"
    / "uganda_mobile_money_708000_10_no_SHAP"
)
...
def strip_pipeline_prefix(params: dict) -> dict:
    """Strip the 'classifier__' prefix GridSearchCV adds for Pipeline steps."""
...
def write_experiment_summary(
    ...
    """Write a text summary of the experiment, including tuning and CV results."""
    ...
    lines = [
        ...
        "",
        "Hyperparameter tuning results (best average precision found):",
    ]
    for model_name, result in tuning_results.items():
        lines.append(f"  {model_name}: best_score={result['best_score']:.4f}")
        lines.append(f"    best_params: {result['best_params']}")

    lines.append("")
    lines.append("Hybrid model (untuned) - single-split validation metrics:")
    for key, value in val_metrics.items():
        lines.append(f"  {key}: {value:.4f}")
    lines.append("")
    lines.append("Hybrid model (untuned) - single-split test metrics:")
...
def main() -> None:
    ...
    xgb_tuning = tune_xgboost(
        ...
        output_dir=OUTPUT_DIR / "tuning" / "xgboost", n_iter=60, n_splits=5,
    )

    LOGGER.info("Tuning logistic regression")
    logreg_tuning = tune_logistic_regression(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        output_dir=OUTPUT_DIR / "tuning" / "logistic_regression", n_splits=5,
    )

    LOGGER.info("Tuning hybrid model")
    hybrid_tuning = tune_hybrid_model(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        output_dir=OUTPUT_DIR / "tuning" / "hybrid", n_trials=20, epochs=60,
    )

    tuning_results = {
        "XGBoost": xgb_tuning,
        "Logistic Regression": logreg_tuning,
        "Hybrid Model": hybrid_tuning,
    }

    # --- 5-fold CV: tuned models ---
    LOGGER.info("Running 5-fold CV: tuned hybrid model")
    hybrid_cv_summary = run_kfold_cv(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        output_dir=OUTPUT_DIR / "kfold_hybrid_tuned", n_splits=5,
        tuned_config=hybrid_tuning["best_config"],
    )

    LOGGER.info("Running 5-fold CV: tuned XGBoost")
    xgb_summary = run_xgboost_baseline(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        temporal_feature_names=temporal_names, static_feature_names=static_names,
        output_dir=OUTPUT_DIR / "xgboost_tuned", n_splits=5,
        hyperparameters=xgb_tuning["best_params"],
    )

    LOGGER.info("Running 5-fold CV: tuned logistic regression")
    logreg_summary = run_logistic_regression_baseline(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        temporal_feature_names=temporal_names, static_feature_names=static_names,
        output_dir=OUTPUT_DIR / "logistic_regression_tuned", n_splits=5,
        hyperparameters=strip_pipeline_prefix(logreg_tuning["best_params"]),
    )

    # --- 5-fold CV: untuned baselines and transformer variants ---
    LOGGER.info("Running 5-fold CV: vanilla LSTM baseline (untuned)")
    ...
    LOGGER.info("Running 5-fold CV: cross-attention fusion hybrid (untuned)")
    cross_attention_summary = run_kfold_cv_cross_attention(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        output_dir=OUTPUT_DIR / "kfold_cross_attention", n_splits=5,
    )

    LOGGER.info("Running 5-fold CV: Transformer-encoder hybrid (untuned)")
    transformer_summary = run_kfold_cv_transformer_encoder(
        X_temporal=X_temporal, X_static=X_static, labels=labels,
        output_dir=OUTPUT_DIR / "kfold_transformer_encoder", n_splits=5,
    )

    model_summaries = {
        "Hybrid LSTM-GRU-Attention-FNN (tuned)": hybrid_cv_summary["pooled_metrics"],
        "XGBoost (tuned)": xgb_summary["pooled_metrics"],
        "Logistic Regression (tuned)": logreg_summary["pooled_metrics"],
        "Vanilla LSTM (untuned)": vanilla_lstm_summary["pooled_metrics"],
        "Cross-Attention Fusion Hybrid (untuned)": cross_attention_summary["pooled_metrics"],
        "Transformer-Encoder Hybrid (untuned)": transformer_summary["pooled_metrics"],
    }
    ...
    chart_path, table_path = compare_all_models(
        model_result_dirs={
            "Hybrid LSTM-GRU-Attention-FNN (tuned)": OUTPUT_DIR / "kfold_hybrid_tuned",
            "XGBoost (tuned)": OUTPUT_DIR / "xgboost_tuned",
            "Logistic Regression (tuned)": OUTPUT_DIR / "logistic_regression_tuned",
            "Vanilla LSTM (untuned)": OUTPUT_DIR / "kfold_vanilla_lstm",
            "Cross-Attention Fusion Hybrid (untuned)": OUTPUT_DIR / "kfold_cross_attention",
            "Transformer-Encoder Hybrid (untuned)": OUTPUT_DIR / "kfold_transformer_encoder",
        },
    ...
    # --- Single-split training/evaluation for the original hybrid model (continuity) ---
    ...
    summary_path = write_experiment_summary(
        ...
        model_summaries, tuning_results,
    )
    ...
    LOGGER.info("Tuning results: %s", OUTPUT_DIR / "tuning")
