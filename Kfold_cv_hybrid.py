"""Stratified k-fold cross-validation for the hybrid LSTM-FNN model.

Produces out-of-fold predictions so that every positive-class borrower
in the dataset is evaluated exactly once, giving far more reliable
precision/recall/F1 estimates than a single ~15% test split does when
the positive class is very small (~2% here, ~69 positive borrowers
total). A single held-out split leaves only ~10 positives to evaluate
on; pooling out-of-fold predictions across all folds evaluates all
~69, roughly 7x more signal.
"""
...
from hybrid_model import F1Score, build_hybrid_model
...
def run_kfold_cv(
    ...
    X_static: np.ndarray,
    labels: np.ndarray,
    output_dir: str | Path = MODELS_DIR / "kfold_hybrid",
    n_splits: int = 5,
    epochs: int = 100,
    batch_size: int = 128,
    ...
    for fold_index, (train_index, holdout_index) in enumerate(
        ...
        # Carve an early-stopping validation slice out of this fold's training data.
    ...
    # Pool out-of-fold predictions: every borrower evaluated exactly once,
    # by whichever fold's model held it out.
...
if __name__ == "__main__":
    raise RuntimeError(
        "Import run_kfold_cv(X_temporal, X_static, labels) and call it after "
