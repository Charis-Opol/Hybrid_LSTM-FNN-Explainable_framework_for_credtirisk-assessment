"""Stratified k-fold cross-validation for the cross-attention fusion variant.
...
from hybrid_cross_attention_model import build_cross_attention_hybrid_model
...
def run_kfold_cv_cross_attention(
    X_temporal: np.ndarray,
    X_static: np.ndarray,
    labels: np.ndarray,
    output_dir: str | Path = MODELS_DIR / "kfold_cross_attention",
    n_splits: int = 5,
    epochs: int = 100,
    batch_size: int = 128,
    random_seed: int = RANDOM_SEED,
) -> dict:
    """Run stratified k-fold CV over the cross-attention fusion model.
    ...
    for fold_index, (train_index, holdout_index) in enumerate(
        ...
        model = build_cross_attention_hybrid_model(
...
if __name__ == "__main__":
    raise RuntimeError(
        "Import run_kfold_cv_cross_attention(X_temporal, X_static, labels) and "
        "call it after building datasets, e.g. from a run_experiment script."
    )
