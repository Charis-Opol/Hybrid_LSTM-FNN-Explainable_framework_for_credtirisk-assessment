"""Stratified k-fold cross-validation for a vanilla LSTM baseline.

Temporal-only sequence model (no static features, no attention, no
GRU-fusion) -- isolates how much the attention mechanism and static
branch in the full hybrid model are actually contributing, versus a
plain recurrent baseline on the same transaction sequences.
"""
...
def build_vanilla_lstm(sequence_length: int, temporal_features: int) -> tf.keras.Model:
    """Build a simple single-layer LSTM classifier (no attention, no static branch)."""

    inputs = tf.keras.Input(shape=(sequence_length, temporal_features), name="temporal_input")
    x = tf.keras.layers.Masking(mask_value=0.0)(inputs)
    x = tf.keras.layers.LSTM(64, dropout=0.3, recurrent_dropout=0.2)(x)
...
def run_kfold_cv_vanilla_lstm(
    X_temporal: np.ndarray,
    labels: np.ndarray,
    output_dir: str | Path = MODELS_DIR / "kfold_vanilla_lstm",
    n_splits: int = 5,
    epochs: int = 100,
    batch_size: int = 128,
    random_seed: int = RANDOM_SEED,
) -> dict:
    """Run stratified k-fold CV over a vanilla LSTM, pooling OOF predictions.
    ...
    for fold_index, (train_index, holdout_index) in enumerate(
        skf.split(X_temporal, labels), start=1
    ):
        LOGGER.info("=== Fold %d/%d ===", fold_index, n_splits)

        X_trainfold = X_temporal[train_index]
...
if __name__ == "__main__":
    raise RuntimeError(
        "Import run_kfold_cv_vanilla_lstm(X_temporal, labels) and call it after "
