...
Train hybrid LSTM-FNN model, evaluate on validation/test sets, and run
stratified k-fold cross-validation for the hybrid model plus an XGBoost
baseline for a more reliable, imbalance-aware comparison.
"""
...
OUTPUT_DIR = (
    PROJECT_DIR
    / "models"
    / "uganda_mobile_money_708000_6_no_SHAP"
)
...
def write_experiment_summary(
    ...
    val_metrics: dict,
    test_metrics: dict,
    hybrid_cv_metrics: dict,
    xgb_metrics: dict,
) -> Path:
    """Write a text summary of the experiment, including CV and baseline results."""
    ...
    lines = [
        ...
        "",
        "Validation metrics (single split):",
    ]
    for key, value in val_metrics.items():
        lines.append(f"  {key}: {value:.4f}")
    lines.append("")
    lines.append("Test metrics (single split):")
    for key, value in test_metrics.items():
        lines.append(f"  {key}: {value:.4f}")
    lines.append("")
    lines.append("Hybrid model - pooled out-of-fold metrics (5-fold CV):")
    ...
    lines.append("")
    lines.append("XGBoost baseline - pooled out-of-fold metrics (5-fold CV):")
...
def main() -> None:
    ...
    # --- Stratified k-fold CV: hybrid model and XGBoost baseline ---
    # Given the small positive class (~2%), these give far more stable
    # metrics than a single train/val/test split by pooling out-of-fold
    # predictions across all folds. Runs before the single-split training
    # below so both results land in the summary together.
    LOGGER.info("Running 5-fold CV for hybrid model")
    hybrid_cv_summary = run_kfold_cv(
        X_temporal=X_temporal,
        X_static=X_static,
        labels=labels,
        output_dir=OUTPUT_DIR / "kfold_hybrid",
        n_splits=5,
    )
    ...
    xgb_summary = run_xgboost_baseline(
        ...
        output_dir=OUTPUT_DIR / "xgboost_baseline",
        n_splits=5,
    )
    ...
    # --- Diagnostics: per-split label distribution ---
    ...
    summary_path = write_experiment_summary(
        ...
        val_metrics,
        test_metrics,
        hybrid_cv_summary["pooled_metrics"],
        xgb_summary["pooled_metrics"],
    )
    ...
    LOGGER.info("Hybrid k-fold CV: %s", OUTPUT_DIR / "kfold_hybrid" / "kfold_summary.json")
    LOGGER.info("XGBoost baseline: %s", OUTPUT_DIR / "xgboost_baseline" / "kfold_summary.json")
