#!/usr/bin/env python
"""Evaluation comparison: hybrid (LSTM cell), logistic regression, XGBoost,
and both transformer variants.

Identical to run_experiment_evaluation_comparison.py except the hybrid
model's temporal encoder uses an LSTM cell instead of GRU (see
run_ablation_gru_vs_lstm.py -- isolating just the cell type showed LSTM
beating GRU on recall/F1/ROC-AUC on this data). Writes to a separate
output folder so the original GRU-based comparison results are not
overwritten.
...
each model family's default settings:
  ...
  1. Hybrid LSTM-Attention-FNN         -- GRU cell swapped for LSTM
...
OUTPUT_DIR = (
    PROJECT_DIR
    / "models"
    / "uganda_mobile_money_evaluation_comparison_lstm"
)
...
def write_experiment_summary(
    ...
    lines = [
        "Evaluation Comparison: Hybrid (LSTM cell) vs. Logistic Regression vs. XGBoost vs. Transformers",
        "=" * 80,
        f"Data source: {DATA_PATH.name}",
        "All models untuned (default hyperparameters). 5-fold CV, pooled OOF metrics.",
        "Hybrid model uses an LSTM temporal encoder cell (see run_ablation_gru_vs_lstm.py).",
        "",
    ]
...
def main() -> None:
    ...
    LOGGER.info("Running 5-fold CV: hybrid LSTM-Attention-FNN (LSTM cell)")
    ...
    model_summaries = {
        "Hybrid LSTM-Attention-FNN": hybrid_summary["pooled_metrics"],
    ...
    chart_path, table_path, pr_overlay_path, lift_table_path = compare_all_models(
        model_result_dirs={
            "Hybrid LSTM-Attention-FNN": OUTPUT_DIR / "kfold_hybrid",
