"""Stratified k-fold cross-validation for the cross-pollinated model
(transformer temporal encoder + hybrid static encoder). Mirrors
Kfold_cv_hybrid.py / transformer_model.py's CV runners exactly (same
class-weight convention, callbacks, OOF pooling) so pooled metrics are
directly comparable to the rest of the model comparison table.
"""
...
def run_kfold_cv_cross_pollinated(
    X_temporal: np.ndarray,
    X_static: np.ndarray,
    labels: np.ndarray,
    output_dir: str | Path = MODELS_DIR / "kfold_cross_pollinated",
    n_splits: int = 5,
    epochs: int = 100,
    batch_size: int = 128,
...
if __name__ == "__main__":
    raise RuntimeError(
        "Import run_kfold_cv_cross_pollinated(X_temporal, X_static, labels) "
